...
#########
try:
    x_right = const.start_move_locate + 20e-6
    print('x_right = const.start_move_locate')
except:
    x_right = const.x_max/2 + 20e-6
...
###########
##########
#savedir = const.txtdir + str(point[0]/1e-6) + 'um' + str(int(R/1e-6)) + 'um'+ 'TimeSequence.npy'
savedir = const.txtdir + str(int(x_right/1e-6)) + 'um'+ 'TimeSequence.npy'
...
def getThetaTField(t,point,R):
    ...
    index = int(x_right/const.delta_x)
    TimeSequence = data[index,:]#addFieldTheta(data,point,R)
    return TimeSequence

...
savefigdir=const.figdir+'ez_Thz_'+'efficiency.png'
savefigdir2=const.figdir+'ez_field_energe.png'
interval = 100
def k_n():
	k_n=[]
...
if os.path.exists(const.txtdir + 'eff_ez.npy') == True:
        sys.exit()
...
np.save(const.txtdir + 'eff_ez.npy',efficiency)
#mapList=np.arange(start,stop+int(stop/interval),int(stop/interval))
...
np.save(const.txtdir + 'eff_locate_ez.npy',locate)

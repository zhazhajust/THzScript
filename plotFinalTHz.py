...
savedir = const.figdir + 'R' +str(int(R/1e-6)) + Field
...
def plotFreqs(Theta,Freq,XF):
    ...
    plt.pcolormesh(np.abs(XF),cmap = plt.cm.jet)
...
def main():
    ...
    plotField(Theta,Time,data)    
    Theta , Freq , XF = getFreqs(data,theta)
    plotFreqs(Theta,Freq,XF)

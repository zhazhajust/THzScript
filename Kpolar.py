...
def draw(theta,K,KKxy,x,Limit,savedir,title,cmap):
    ...
    try:
        ...
        print('error at KKxy[K<Limit].max(),np.argmax(KKxy[K<Limit])')
    plt.scatter(theta[K<Limit][K[K<Limit]>0.1],K[K<Limit][K[K<Limit]>0.1],s=5,marker = ',',c=KKxy[K<Limit][K[K<Limit]>0.1],cmap=cmap)#,alpha= 0.7)
    #plt.scatter(theta,K,s=0.1,marker = ',',c=KKxy**2,cmap=cmap)#,alpha= 0.7)

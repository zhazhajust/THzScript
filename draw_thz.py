...
def draw(x):
	...
	'''
	for x in range(0,const.Nx/2):
		for y in range(0,const.Ny/2):
			if math.sqrt(x**2+y**2)<k[0]:
				k_bz[x,y]=0
			if math.sqrt(x**2+y**2)>k[-1]:
				k_bz[x,y]=0
	'''		
...
middle = int(middle)
middle = int(middle/10)*10
import find_nearest
middle=find_nearest(midlle)
draw(middle)
#draw(n_sdf)

...
from lib.bz.field_filter import Bz_field_filter
...
savefigdir=const.figdir+'BzThz'+'Efficiency.png'
...
if os.path.exists(const.txtdir + 'EffBz.npy') == True:
        sys.exit()
...
def task(x):
	list=Bz_field_filter(x,const.Nx,const.Ny,const.delta_x,const.figdir,const.sdfdir,const.filenumber)
	return list
pool = multiprocessing.Pool(processes=96)
mapList=np.arange(start,stop+int(stop/interval),int(stop/interval))
final_energe = pool.map(task,mapList)

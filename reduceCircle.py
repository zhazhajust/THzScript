...
@njit
def getIndex(theta):
    intervalTheta = np.linspace(-90,90,2000)
...
def plotFreqsbak(Theta,Freq,XF,savedir):
    ...
    #XXF,Theta,Freq = interpFreq(Theta,Freq,XF)


    #XXF = transform.resize(np.abs(XF), (1000, 2000),order=3)
...
def main():
    ...
    np.save(const.txtdir + 'reducedCircle.npy',data)
    np.save(const.txtdir + 'Circletheta.npy',theta)
